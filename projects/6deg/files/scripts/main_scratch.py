...
start_username = "NikiTricky"
outfile = "followers.dot"
degrees = 5
...
def get_followers(username):
    # Make scratch URL
    url = "https://scratch.mit.edu/users/" + username + "/followers/"
    driver.get(url)

    # Get all followers
    follower_els = driver.find_elements(by=By.CSS_SELECTOR, value="li.user.thumb.item span.title a")
    followers = []

    for follower_el in follower_els:
        followers.append(follower_el.get_attribute("href").split("/")[4])
    ...
    while True:
        next_page_link = driver.find_elements(by=By.CSS_SELECTOR, value="span.page-links a:last-child")[-1]
...
def follower_tree(username, degrees):
    ...
    if degrees == 0:
        ...
        with open("edge_users.txt", "a") as f:
            f.write(username + "\n")
        return
